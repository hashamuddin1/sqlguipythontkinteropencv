...
def call_me1():
    ...
    def sql2():
        sql_a=Tk()
        sql_a.title("CUSTOMER HAVING JOURNEY ON OCTOBER")
        sql_a.geometry("1340x680+0+0")
        ...
        import tkinter as tk
        from tkinter import ttk
        sql="SELECT * FROM customer5 where amount_of_payment>1000"
        cursor.execute(sql)
        rows=cursor.fetchall()
        frn=Frame(sql_a)
        frn.pack(side=tk.LEFT,padx=0)
    ...
    buttonFont = font.Font(family='Helvetica', size=26, weight='bold')
    bbb=Button(window,text="CUSTOMER HAVING JOURNEY ON OCTOBER",command=sql2,font=buttonFont)
    bbb.pack()
